...
function initFirebaseListeners() {
  ...
  // Listen for Settings
  window.unsubscribers.push(window.fbOnSnapshot(window.fbDoc(window.db, 'settings', 'store'), (doc) => {
    if (doc.exists()) {
      DB.settings = doc.data();
      localStorage.setItem('syncro_db', JSON.stringify(DB));
      syncGlobalLogo(DB.settings.logoUrl, DB.settings.bizName);
      if (currentPage === 'landing') renderLanding();
    }
  }));
...
function navigate(page) {
  ...
  const titles = { dashboard: 'Dashboard', products: 'Menu & Produk', ingredients: 'Master Bahan', recipes: 'Resep & HPP', orders: 'Invoice', pos: 'Point of Sale', procurement: 'Pengadaan Bahan', expenses: 'Biaya Operasional', reports: 'Laporan', customers: 'Data Pelanggan', landing: 'Pengaturan Landing Page', profile: 'Profil', settings: 'Pengaturan' };
  document.getElementById('topbar-title').textContent = titles[page] || page;
  const content = document.getElementById('main-content');
  content.innerHTML = '';
  closeSidebar();
  const pages = { dashboard: renderDashboard, products: renderProducts, ingredients: renderIngredients, recipes: renderRecipes, orders: renderOrders, pos: renderPOS, procurement: renderProcurement, expenses: renderExpenses, reports: renderReports, customers: renderCustomers, landing: renderLanding, profile: renderProfile, settings: renderSettings };
...
// ===================== INIT =====================
function syncGlobalLogo(url, bizName = 'Syncro') {
  const fallback = `https://ui-avatars.com/api/?name=${encodeURIComponent((bizName || 'SS').substring(0, 2).toUpperCase())}&background=0F6A4A&color=fff&rounded=true`;
  const finalUrl = (url && url.trim() !== '') ? url : fallback;
  const targets = ['sidebar-brand-logo', 'auth-brand-logo', 'header-brand-logo'];
  targets.forEach(id => {
    const el = document.getElementById(id);
    if (el) { el.src = finalUrl; el.style.display = 'block'; }
  });
}
...
function getValidOrders() {
  // Business Rule: Mengabaikan invoice yang dibatalkan
  return DB.orders.filter(order => order.status !== 'dibatalkan' && order.paymentStatus !== 'dibatalkan');
}
...
function openLandingPage() {
  window.open(' https://aswinm-shuai.github.io/landingpage-srisoengkem/', '_blank');
}
...
function openOrderPage() {
  window.open(' https://aswinm-shuai.github.io/landingpage-srisoengkem/#menu', '_blank');
}
...
function submitOrder() {
  ...
  const order = { id: 'o' + Date.now(), date: new Date().toISOString(), customerName: name, wa, address: addr, mapsLink: document.getElementById('cust-maps')?.value || '', note: document.getElementById('cust-note')?.value || '', items, total, payment: 'transfer', paymentMethod: null, status: 'menunggu', paymentStatus: 'belum_bayar' };
...
function showProductModal(id = null) {
  ...
  showModal(`
    ...
    <div class="form-group"><label class="form-check" style="display:flex; align-items:center; gap:12px;"><input type="checkbox" id="pf-avail" class="toggle-switch" ${p.available ? 'checked' : ''}><span>Produk tersedia (tampil di menu)</span></label></div>
...
function openCloudinaryWidget(targetId = 'pf-img') {
...
// ===================== INGREDIENTS =====================
function getIngredientPriceDisplay(ing) {
  if (!ing) return 'Belum Ada Data';
  let latestProcDate = '';
  let latestPrice = 0;
  DB.procurement.forEach(p => {
    p.items.forEach(it => {
      let match = (it.ingId === ing.id);
      if (!match && it.name && ing.name) match = it.name.trim().toUpperCase() === ing.name.trim().toUpperCase();
      if (match && it.qty > 0 && it.totalPrice > 0) {
        if (!latestProcDate || p.date >= latestProcDate) {
          latestProcDate = p.date;
          const convertedQty = getConvertedQty(it.qty, it.unit, ing.unit);
          if (convertedQty > 0) latestPrice = it.totalPrice / convertedQty;
        }
      }
    });
  });
  if (!latestProcDate) return 'Belum Ada Data';
  return `Rp ${latestPrice % 1 === 0 ? latestPrice : latestPrice.toFixed(1)} / ${ing.unit}`;
}
...
function renderIngredients() {
  document.getElementById('main-content').innerHTML = `
    ...
    <div class="card"><div class="table-wrap responsive-table"><table><thead><tr><th>Kode</th><th>Nama Bahan</th><th>Satuan</th><th>Harga Satuan Terkecil Terakhir</th><th>Aksi</th></tr></thead><tbody id="ing-tbody"></tbody></table></div></div>
  `;
...
function renderIngTable(ings) {
  ...
  tb.innerHTML = ings.map(i => `<tr>
    <td data-label="Kode"><span class="badge badge-gray">${i.code || '-'}</span></td>
    <td data-label="Nama Bahan"><strong>${i.name}</strong></td>
    <td data-label="Satuan">${i.unit}</td>
    <td data-label="Harga Satuan Terkecil Terakhir"><strong>${getIngredientPriceDisplay(i)}</strong></td>
    <td data-label="Aksi"><div style="display:flex;gap:4px"><button class="btn-icon" onclick="showIngModal('${i.id}')" aria-label="Edit"><i class="fas fa-edit"></i></button><button class="btn-icon" onclick="deleteIng('${i.id}')" style="color:var(--danger)"><i class="fas fa-trash"></i></button></div></td>
  </tr>`).join('') || `<tr><td colspan="5" style="text-align:center;color:var(--gray-400);padding:32px">Belum ada bahan</td></tr>`;
}
...
function showIngModal(id = null) {
  editingId = id;
  const i = id ? DB.ingredients.find(x => x.id === id) : { name: '', unit: 'gram', price: 0, code: '' };
  showModal(`
...
function saveIng() {
  ...
  const data = { name, unit: document.getElementById('if-unit').value, stock: editingId ? DB.ingredients.find(x => x.id === editingId).stock : 0 };
  let newIng;
...
function renderRecipes() {
  ...
  g.innerHTML = DB.products.map(p => {
    ...
    return `<div class="card" style="position:relative">
      ...
      <div style="display:flex;gap:8px">
        <button class="btn-secondary btn-sm" style="flex:1" onclick="showRecipeModal('${p.id}')"><i class="fas fa-${recipe ? 'edit' : 'plus'}"></i> ${recipe ? 'Edit' : 'Buat Resep'}</button>
        ${recipe ? `<button class="btn-primary btn-sm" style="flex:1" onclick="showHppDetail('${p.id}')"><i class="fas fa-list"></i> Detail HPP</button>` : ''}
      </div>
    </div>`;
  }).join('');
}
function showHppDetail(productId) {
  ...
  const marginObj = ((p.price - hpp) / p.price * 100);
  const margin = p.price > 0 && !isNaN(marginObj) ? marginObj.toFixed(1) : 0;
  const laba = p.price - hpp;

  const ingredientsHtml = recipe.items.map(ri => {
    const ing = DB.ingredients.find(i => i.id === ri.ingId);
    return ing ? `<div style="display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid var(--gray-100);font-size:13px">
          <div><div style="font-weight:600">${ing.name}</div><div style="font-size:11px;color:var(--gray-500)">${ri.qty} ${ing.unit}</div></div>
          <div style="font-weight:600;color:var(--emerald)">${fmt(ing.price * ri.qty)}</div>
        </div>` : '';
  }).join('');

  showModal(`
        <div class="modal-header"><div class="modal-title">Detail HPP: ${p.name}</div><button class="btn-icon" onclick="closeModal()"><i class="fas fa-times"></i></button></div>
        <div class="grid-3" style="margin-bottom:16px;text-align:center">
          <div style="background:var(--gray-50);padding:10px;border-radius:var(--radius)">
            <div style="font-size:11px;color:var(--gray-500)">TOTAL HPP</div><div style="font-weight:700;color:var(--emerald)">${fmt(hpp)}</div>
          </div>
          <div style="background:var(--gray-50);padding:10px;border-radius:var(--radius)">
            <div style="font-size:11px;color:var(--gray-500)">ESTIMASI LABA</div><div style="font-weight:700;color:var(--emerald)">${fmt(laba)}</div>
          </div>
          <div style="background:var(--gray-50);padding:10px;border-radius:var(--radius)">
            <div style="font-size:11px;color:var(--gray-500)">MARGIN</div><div style="font-weight:700;color:var(--emerald)">${margin}%</div>
          </div>
        </div>
        <div style="font-size:12px;font-weight:600;color:var(--gray-500);margin-bottom:8px">KOMPOSISI BAHAN</div>
        <div style="max-height:250px;overflow-y:auto;margin-bottom:16px;padding-right:4px">
          ${ingredientsHtml}
        </div>
        <div class="modal-footer"><button class="btn-secondary" onclick="closeModal()">Tutup</button></div>
      `);
}
...
function renderPOS() {
      ...
      <div style="display:flex;flex-direction:column">
        <div class="card" style="flex:1;display:flex;flex-direction:column;overflow:hidden">
          ...
          <div style="border-top:1px solid var(--gray-200);padding-top:12px;margin-top:8px">
            ...
            <button class="btn-primary" style="width:100%;padding:12px" onclick="doCheckoutPOS()"><i class="fas fa-file-invoice" style="margin-right:6px"></i>Buat Invoice</button>
  ...
  // Cleanup old floating cart if navigating away, re-inject fresh
  const existingFC = document.getElementById('mobile-floating-cart');
  if (existingFC) existingFC.remove();
  updatePOSTotal(0); // trigger re-creation
}
let posCart = {};
...
function updatePOSTotal(total) {
  ...
  if (s) s.textContent = fmt(total);
  if (t) t.textContent = fmt(total);

  // Enhanced Mobile Floating Cart
  let floatingCart = document.getElementById('mobile-floating-cart');

  // Create if not exists (first call during renderPOS)
  if (!floatingCart && currentPage === 'pos') {
    floatingCart = document.createElement('div');
    floatingCart.id = 'mobile-floating-cart';
    floatingCart.className = 'hidden';
    floatingCart.innerHTML = `
      <div class="fc-left">
        <span>🛒</span>
        <span class="fc-badge" id="fc-item-count">0 Item</span>
      </div>
      <span class="fc-total" id="fc-total-price">Rp 0</span>
    `;
    floatingCart.addEventListener('click', () => {
      // Show cart panel or scroll to it
      const cartPanel = document.querySelectorAll('#main-content > div > div')[1];
      if (cartPanel) {
  ...
  if (floatingCart) {
    const itemsCount = Object.values(posCart).reduce((a, b) => a + b, 0);
    const fcCount = document.getElementById('fc-item-count');
    const fcTotal = document.getElementById('fc-total-price');

    if (itemsCount > 0) {
      if (fcCount) fcCount.textContent = `${itemsCount} Item`;
      if (fcTotal) fcTotal.textContent = fmt(total);
...
function doCheckoutPOS() {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    const p = DB.products.find(x => x.id === id);
    if (p) {
      orderItems.push({ productId: id, qty: qty, price: p.price });
      txItems.push({ productId: id, qty: qty, price: p.price, productName: p.name, hpp: calcHPP(id) });
    }
  }
  ...
  saveDB(); clearPOSCart();
  toast(`Invoice Rp${total.toLocaleString()} berhasil dibuat!`, 'success');
  renderPOS();
}
...
function renderOrders() {
  ...
  document.getElementById('main-content').innerHTML = `
    ...
    <div class="tabs">
      ...
      <div class="tab" onclick="switchOrderTab('belum_bayar',this)">Belum Bayar</div>
      <div class="tab" onclick="switchOrderTab('lunas',this)">Lunas</div>
      <div class="tab" onclick="switchOrderTab('tunai',this)">Tunai</div>
      <div class="tab" onclick="switchOrderTab('qris',this)">QRIS</div>
    </div>
    <div class="card"><div class="table-wrap responsive-table">
      <table>
        <thead>
          <tr>
            <th>No. Invoice</th>
...
function renderOrderList(status) {
  ...
  const orders = status === 'semua' ? filteredByDate : filteredByDate.filter(o => {
    if (status === 'belum_bayar' || status === 'lunas') return o.paymentStatus === status;
    if (status === 'tunai' || status === 'qris') return o.paymentMethod === status;
    return o.status === status;
  });
  const el = document.getElementById('orders-list');
  if (!orders.length) { el.innerHTML = '<tr><td colspan="9" style="text-align:center;color:var(--gray-400);padding:32px"><i class="fas fa-inbox" style="margin-right:8px"></i>Tidak ada invoice</td></tr>'; return }
  el.innerHTML = orders.slice().reverse().map(o => {
    const invNo = 'INV-' + o.id.substring(0, 5).toUpperCase();
    let paymentBadge = `<span class="badge" style="background:#FEF3C7;color:#92400E">BELUM BAYAR</span>`;
    if (o.paymentStatus === 'lunas') paymentBadge = `<span class="badge" style="background:#DCFCE7;color:#166534">LUNAS</span>`;

    let methodBadge = '-';
    if (o.paymentMethod === 'tunai') methodBadge = `<span class="badge badge-gray">Tunai</span>`;
    if (o.paymentMethod === 'qris') methodBadge = `<span class="badge badge-info">QRIS</span>`;

    const isLandingPage = (o.source === 'landing-page' || o.orderSource === 'landing-page' || o.googleMapsUrl || o.shareLocation || o.latitude || o.longitude || o.mapsUrl);
    return `<tr>
      <td data-label="No. Invoice"><strong>${invNo}</strong></td>
      <td data-label="Pelanggan"><div style="font-weight:500">${o.customerName}</div></td>
      <td data-label="Source"><div style="font-size:12px">${isLandingPage ? '🌐 Landing Page' : '🛒 POS'}</div></td>
      <td data-label="Total" style="font-weight:600;color:var(--emerald)">${fmt(o.total)}</td>
      <td data-label="Status Bayar">${paymentBadge}</td>
      <td data-label="Metode Bayar">${methodBadge}</td>
      <td data-label="Status Pesanan"><span class="badge ${o.status === 'selesai' ? 'badge-success' : o.status === 'pending' || o.status === 'menunggu' ? 'badge-warning' : 'badge-info'}">${o.status}</span></td>
      <td data-label="Tanggal" style="font-size:12px;color:var(--gray-500)">${new Date(o.date).toLocaleString('id-ID')}</td>
      <td data-label="Aksi">
        <div style="display:flex;gap:4px">
          <button class="btn-primary btn-sm" onclick="showInvoiceDetail('${o.id}')" aria-label="Detail"><i class="fas fa-eye"></i></button>
          <button class="btn-delete-invoice" onclick="cancelInvoice('${o.id}')" title="Hapus Invoice"><i class="fas fa-trash"></i></button>
        </div>
      </td>
    </tr>`;
  }).join('');
...
async function cancelInvoice(id) {
  if (confirm('Apakah Anda yakin ingin menghapus invoice ini?')) {
    try {
      if (window.db) {
        await window.fbWriteBatch(window.db).delete(window.fbDoc(window.db, 'orders', id)).commit();
      } else {
        DB.orders = DB.orders.filter(o => o.id !== id);
        saveDB();
        renderOrders();
      }
      toast('Transaksi di-VOID dan dihapus', 'success');
    } catch (e) {
      console.error(e);
...
async function saveInvoiceValidation(id) {
  const pMethod = document.getElementById('inv-payment-method').value;
  const pStatus = document.getElementById('inv-payment-status').value;
  const oStatus = document.getElementById('inv-order-status').value;
  const o = DB.orders.find(x => x.id === id);
  if (!o) return;

  let finalMethod = pMethod;
  const isLandingPage = (o.source === 'landing-page' || o.orderSource === 'landing-page' || o.googleMapsUrl || o.shareLocation || o.latitude || o.longitude || o.mapsUrl);
  if (isLandingPage) finalMethod = 'qris';

  if (pStatus === 'lunas' && !finalMethod) {
    toast('Pilih metode pembayaran terlebih dahulu.', 'warning');
    return;
  }

  const updateData = {
    paymentMethod: finalMethod || null,
  ...
  if (pStatus === 'lunas') {
    ...
    updateData.validatedBy = currentUser?.email || 'admin';

    // Generate Transaction if missing
    if (o.paymentStatus !== 'lunas' && !DB.transactions.some(t => t.orderId === o.id)) {
  ...
  try {
    if (window.db) {
      await window.fbSetDoc(window.fbDoc(window.db, 'orders', id), updateData, { merge: true });
    } else {
    ...
    toast('Validasi pembayaran berhasil disimpan', 'success');
    closeModal();
  } catch (e) {
    console.error(e);
...
async function deleteInvoice(id) {
  if (!currentUser || (currentUser.role !== 'admin' && currentUser.role !== 'owner')) {
    toast('Hanya Admin/Owner yang dapat menghapus invoice', 'error');
    return;
  }
  if (confirm('Peringatan: Data yang dihapus tidak dapat dikembalikan. Lanjutkan hapus invoice?')) {
    try {
      if (window.db) {
        await window.fbDoc(window.db, 'orders', id); // just checking
        await window.fbWriteBatch(window.db).delete(window.fbDoc(window.db, 'orders', id)).commit();
      } else {
        DB.orders = DB.orders.filter(o => o.id !== id);
        saveDB();
      ...
      console.error(e);
      toast('Gagal menghapus invoice', 'error');
    }
  }
}
...
function showInvoiceDetail(id) {
  ...
  if (!o) return;
  const invNo = 'INV-' + o.id.substring(0, 5).toUpperCase();
  const prod_html = o.items.map(it => { const p = DB.products.find(x => x.id === it.productId); return `<div style="display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid var(--gray-100)"><span>${p?.name || '?'} x${it.qty}</span><span style="font-weight:600">${fmt(it.price * it.qty)}</span></div>`; }).join('');

  const isLandingPage = (o.source === 'landing-page' || o.orderSource === 'landing-page' || o.googleMapsUrl || o.shareLocation || o.latitude || o.longitude || o.mapsUrl);

  showModal(`
        <div class="modal-header"><div class="modal-title">Detail Invoice ${invNo}</div><button class="btn-icon" onclick="closeModal()"><i class="fas fa-times"></i></button></div>
        <div style="margin-bottom:16px;">
          <span class="badge ${isLandingPage ? 'badge-info' : 'badge-gray'}">${isLandingPage ? '🌐 Order dari Landing Page' : '🛒 Order dari POS'}</span>
        </div>
        <div class="grid-2" style="margin-bottom:16px">
          <div><div style="font-size:12px;color:var(--gray-500)">Pelanggan</div><div style="font-weight:600">${o.customerName}</div></div>
          <div><div style="font-size:12px;color:var(--gray-500)">Tanggal</div><div>${new Date(o.date).toLocaleString('id-ID')}</div></div>
        </div>
        ${o.address ? `<div style="margin-bottom:16px;font-size:13px"><i class="fas fa-map-marker-alt" style="color:var(--danger);margin-right:6px"></i>${o.address}</div>` : ''}
        ${o.maps ? `<div style="margin-bottom:16px"><a href="${o.maps}" target="_blank" class="btn-secondary btn-sm" style="display:inline-block"><i class="fas fa-location-arrow"></i> Buka Peta</a></div>` : ''}
        ${(o.googleMapsUrl || o.shareLocation || o.mapsUrl || o.mapsLink || o.locationLink) ? `
          <div style="margin-bottom:16px;padding:12px;background:var(--gray-50);border-radius:var(--radius)">
            <div style="font-weight:600;margin-bottom:8px">Lokasi Customer</div>
            <div style="font-size:13px;margin-bottom:8px"><i class="fas fa-map-marker-alt" style="color:var(--danger);margin-right:6px"></i>${o.address || '-'}</div>
            <button class="btn-secondary btn-sm" onclick="window.open('${o.googleMapsUrl || o.shareLocation || o.mapsUrl || o.mapsLink || o.locationLink}','_blank')"><i class="fas fa-map"></i> Lihat di Google Maps</button>
          </div>
        ` : ''}
        <div style="background:var(--gray-50);padding:12px;border-radius:var(--radius);margin-bottom:16px">
          <div style="font-weight:600;margin-bottom:8px">Item Pesanan</div>
          ${prod_html}
          <div style="display:flex;justify-content:space-between;margin-top:12px;font-weight:700;font-size:16px;color:var(--emerald)"><span>Total</span><span>${fmt(o.total)}</span></div>
        </div>
        ${o.notes ? `<div class="alert alert-warning" style="margin-bottom:16px"><i>Catatan: ${o.notes}</i></div>` : ''}
        <div class="grid-2">
          <div class="form-group"><label class="form-label">Metode Pembayaran</label>
            <select id="inv-payment-method" ${isLandingPage ? 'disabled' : ''}>
              <option value="" ${!o.paymentMethod && !isLandingPage ? 'selected' : ''}>Belum Dipilih</option>
              <option value="tunai" ${o.paymentMethod === 'tunai' ? 'selected' : ''}>Tunai</option>
              <option value="qris" ${o.paymentMethod === 'qris' || isLandingPage ? 'selected' : ''}>QRIS</option>
            </select>
          </div>
          <div class="form-group"><label class="form-label">Status Pembayaran</label>
            <select id="inv-payment-status">
              <option value="belum_bayar" ${o.paymentStatus === 'belum_bayar' ? 'selected' : ''}>Belum Bayar</option>
        ...
        <div style="margin-bottom:16px"><button class="btn-primary" style="width:100%" onclick="saveInvoiceValidation('${o.id}')">Validasi & Simpan</button></div>
        <div class="modal-footer" style="justify-content:space-between">
          <div>
            ${currentUser && (currentUser.role === 'admin' || currentUser.role === 'owner') ? `<button class="btn-secondary" style="color:var(--danger);border-color:var(--danger)" onclick="deleteInvoice('${o.id}')"><i class="fas fa-trash"></i> Hapus Permanen</button>` : ''}
          </div>
...
function updateOrderStatus(id, status) {
  ...
  if (o) {
    o.status = status;
    if (status === 'selesai' && !DB.transactions.some(t => t.orderId === o.id)) {
      o.paymentStatus = 'lunas';
      const txItems = o.items.map(it => {
        const p = DB.products.find(x => x.id === it.productId);
        return { productId: it.productId, qty: it.qty, price: it.price, productName: p?.name || 'Produk Dihapus', hpp: p ? calcHPP(p.id) : 0 };
...
function renderCustomers() {
  ...
  <div class="card"><div class="table-wrap responsive-table"><table><thead><tr><th>Nama</th><th>WhatsApp</th><th>Alamat</th><th>Total Order</th><th>Total Belanja</th><th>Aksi</th></tr></thead><tbody id="cust-tbody"></tbody></table></div></div>
  `;
...
function renderCustTable(custs) {
  ...
  tb.innerHTML = custs.map(c => `<tr>
    <td data-label="Nama"><div style="font-weight:500">${c.name}</div></td>
    <td data-label="WhatsApp"><a href="https://wa.me/${c.wa?.replace(/\D/g, '')}" target="_blank" style="color:var(--emerald)">${c.wa || '-'}</a></td>
    <td data-label="Alamat" style="max-width:150px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${c.address || '-'}</td>
    <td data-label="Total Order">${c.totalOrders || 0}x</td>
    <td data-label="Total Belanja" style="font-weight:600;color:var(--emerald)">${fmt(c.totalSpent || 0)}</td>
    <td data-label="Aksi"><div style="display:flex;gap:4px"><button class="btn-icon" onclick="showCustModal('${c.id}')" aria-label="Edit"><i class="fas fa-edit"></i></button><button class="btn-icon" onclick="deleteCust('${c.id}')" style="color:var(--danger)"><i class="fas fa-trash"></i></button></div></td>
  ...
  document.getElementById('main-content').innerHTML = `
    <div class="grid-2" style="gap:24px">
      <div>
        ...
        <div class="card" style="margin-bottom:16px">
          <div class="card-title" style="margin-bottom:16px">Pengaturan Gambar Landing Page</div>
          
          <div class="form-group" style="margin-bottom:24px">
            <label class="form-label">A. Hero / Beranda Image</label>
            <div style="font-size:12px;color:var(--gray-500);margin-bottom:8px;">Gambar utama yang tampil pada halaman Beranda Landing Page.</div>
            <div style="display:flex;gap:8px;">
              <input type="text" id="lp-hero-img" value="${DB.settings.heroImage || ''}" placeholder="URL Gambar / Kosongkan untuk bawaan">
              <button class="btn-secondary" onclick="openCloudinaryWidget('lp-hero-img')" style="white-space:nowrap"><i class="fas fa-upload"></i> Ubah Gambar</button>
            </div>
          </div>

          <div class="form-group" style="margin-bottom:24px;border-top:1px solid var(--gray-200);padding-top:16px">
            <label class="form-label">B. Kenapa Memilih Kami Image</label>
            <div style="font-size:12px;color:var(--gray-500);margin-bottom:8px;">Gambar yang tampil pada section Kenapa Memilih Kami.</div>
            <div style="display:flex;gap:8px;">
              <input type="text" id="lp-why-img" value="${DB.settings.whyImage || ''}" placeholder="URL Gambar / Kosongkan untuk bawaan">
              <button class="btn-secondary" onclick="openCloudinaryWidget('lp-why-img')" style="white-space:nowrap"><i class="fas fa-upload"></i> Ubah Gambar</button>
            </div>
          </div>
          
          <div class="form-group" style="margin-bottom:24px;border-top:1px solid var(--gray-200);padding-top:16px">
            <label class="form-label">C. Logo Aplikasi (Utama)</label>
            <div style="font-size:12px;color:var(--gray-500);margin-bottom:8px;">Logo utama yang digunakan di Landing Page dan Dashboard Admin.</div>
            <div style="display:flex;gap:8px;">
              <input type="text" id="lp-app-logo" value="${DB.settings.logoUrl || ''}" placeholder="URL Gambar / Kosongkan untuk bawaan">
              <button class="btn-secondary" onclick="openCloudinaryWidget('lp-app-logo')" style="white-space:nowrap"><i class="fas fa-upload"></i> Ubah Logo</button>
            </div>
          </div>

          <div class="form-group" style="margin-bottom:24px;border-top:1px solid var(--gray-200);padding-top:16px">
            <label class="form-label">D. Logo Khusus Navbar Landing Page (Opsional)</label>
            <div style="font-size:12px;color:var(--gray-500);margin-bottom:8px;">Prioritas utama untuk Navbar Landing Page. Jika kosong, akan menggunakan Logo Utama.</div>
            <div style="display:flex;gap:8px;">
              <input type="text" id="lp-landing-logo" value="${DB.settings.landingLogoUrl || ''}" placeholder="URL Gambar / Kosongkan agar otomatis">
              <button class="btn-secondary" onclick="openCloudinaryWidget('lp-landing-logo')" style="white-space:nowrap"><i class="fas fa-upload"></i> Ubah Logo Navbar</button>
            </div>
          </div>
          
          <div style="font-size:12px;color:var(--gray-400);margin-bottom:16px;"><i class="fas fa-info-circle"></i> Format yang didukung: JPG, JPEG, PNG.</div>
...
function saveLandingSettings() {
  ...
  DB.settings.heroImage = document.getElementById('lp-hero-img').value;
  DB.settings.whyImage = document.getElementById('lp-why-img').value;
  DB.settings.logoUrl = document.getElementById('lp-app-logo').value;
  DB.settings.landingLogoUrl = document.getElementById('lp-landing-logo').value;
